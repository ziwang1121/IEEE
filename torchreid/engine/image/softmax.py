...
class MultiModalImageSoftmaxEngine(Engine):
    ...
    #             imgs[i] = imgs[i].cuda()
    #         pids = pids.cuda()
    #         timeids = timeids.cuda()

    #     # outputs = self.model(imgs)
    #     # outputs_R = self.model(imgs)
    #     outputs_R, outputs_N, outputs_T, outputs_time = self.model(imgs, timeids)
    #     # outputs_R, outputs_N = self.model(imgs)
    #     # outputs_R = outputs[0]
    #     # outputs_N = outputs[1]
    #     # outputs_T = outputs[2]
    #     # outputs_RT = outputs[3]
    #     # outputs_RN = outputs[4]

    #     loss_R = self.compute_loss(self.criterion, outputs_R, pids)
    #     loss_N = self.compute_loss(self.criterion, outputs_N, pids)
    #     loss_T = self.compute_loss(self.criterion, outputs_T, pids)
    #     loss_time = self.compute_loss(self.criterion, outputs_time, timeids)

    #     loss = loss_R + loss_N + loss_T + loss_time*2
    #     # loss = loss_R
    ...
    #     acc_time = metrics.accuracy(outputs_time, timeids)[0]

    #     loss_summary = {
    #         'loss_all': loss.item(),
    #         'loss_time': loss_time.item(),
    #         'acc_time': acc_time.item(),
    #         'loss_R': loss_R.item(),
    #         'acc_R': acc_R.item(),
    #         'loss_N': loss_N.item(),
    #         'acc_N': acc_N.item(),
