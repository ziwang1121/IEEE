...
class MLFN3modal(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        super(MLFN3modal, self).__init__()
        self.modal_number = 3
        self.loss = loss
        ...
        self.backbone = nn.ModuleList(
            [
                mlfn(num_classes, pretrained=True)
                for _ in range(self.modal_number)
            ]
        )
        # identity classification layer

        self.classifier_RGB = nn.Linear(1024, num_classes)
        self.classifier_TI = nn.Linear(1024, num_classes)
        self.classifier_NI = nn.Linear(1024, num_classes)
...
def mlfn3modal(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = MLFN3modal(
