...
class MultiModalImageDataset(Dataset):
    ...
    def __init__(self, train, query, gallery, **kwargs):
        super(MultiModalImageDataset, self).__init__(train, query, gallery, **kwargs)
    def __getitem__(self, index):
        img_path, pid, camid, timeid = self.data[index]
        # print(img_path)
        imgs = []
        for i in range(len(img_path)):
            imgs.append(read_image(img_path[i]))
        if self.transform is not None:
            for i in range(len(img_path)):
                imgs[i] = self._transform_image(self.transform, self.k_tfm, imgs[i])
        item = {
            'img': imgs,
            'pid': pid,
            'camid': camid,
