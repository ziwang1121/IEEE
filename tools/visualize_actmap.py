...
def visactmap(
    ...
    for target in list(test_loader.keys()):
        ...
        actmap_dir = osp.join(save_dir, 'actmap_vis_' + save_name)
        ...
        for batch_idx, data in enumerate(data_loader):
            ...
            try:
                outputs_list = model(imgs, return_featuremaps=True)
                if modal == 'RGB':
...
def main():
    ...
    parser.add_argument('--root', type=str, default=r'E:\reidCode\AAAI 2022-master\dataset')
    parser.add_argument('-d', '--dataset', type=str, default='RGBNT201')
    parser.add_argument('-m', '--model', type=str, default='ieee3modalBase')
    parser.add_argument('--weights', type=str)
    parser.add_argument('--save-dir', type=str, default=r'log\actmap')
    ...
    visactmap(
        model, test_loader, args.save_dir, args.save_name, args.width, args.height, use_gpu=use_gpu, modal=args.modal
    )
